...
export default function EditCompraPage() {
  ...
  const [compraToUpdate, setCompraToUpdate] = useState<Compra>();
  ...
  useEffect(() => {
    setHeaderTitle("Compras / Editar");
  }, [setHeaderTitle]);
  const onSubmit = useCallback(async () => {
    try {
      await CompraAPI.update(compraToUpdate!.id, itensCompra);
  ...
  }, [router, itensCompra, compraToUpdate]);
  const getById = useCallback(async () => {
    if (!isNaN(+id)) {
      const compraToUpdate = await CompraAPI.getById(+id);
      setItensCompra(compraToUpdate.itensCompra || []);
      setCompraToUpdate(compraToUpdate);
    }
  }, [id]);
  useEffect(() => {
    async function init() {
